...
router.get("/getUnQualifiedScoresHistory",(req,res,next)=>{
  console.log(req.query)
  const result = getUnQualifiedScoresHistory(req.query);
  return result.then(data=>{
...
router.get("/getUnQualifiedSpecialRecord",(req,res,next)=>{
  const result = getUnQualifiedSpecialRecord(req.query);
  return result.then(data=>{
    
    res.json(new SuccessModel(data));
  })
})
router.get("/getAbsenceCourseNum",(req,res,next)=>{
  const result = getAbsenceCourseNum(req.query);
  return result.then(data=>{
    res.json(new SuccessModel(data));
  })
})
